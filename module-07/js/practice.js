...
//   tweets.reduce(
//     (allTags, tweet) =>
//       tweet.tags.length > 0 ? allTags.concat(tweet.tags) : allTags,
//     [],
//   );
...
// keys.forEach(key => console.log(`${key}: ${tagCount[key]}`));
...
//   arr.reduce((total, element) => total + element, 0);
...
// const getPropValues = (arr, key) => arr.map(element => element[key]);
...
// const setGuestState = (arr, days) =>
//   arr.map(element =>
//     element.inactiveDays > days
...
// const getActiveGuests = arr => arr.filter(element => element.isActive);
...
//   arr.filter(element => element.age > age);
...
// const getTotalPrice = (allProducts, order) =>
//   Object.entries(order).reduce(
//     (acc, [key, value]) => acc + value * allProducts[key],
//     0,
//   );
